          ...
          for k in range(0,epochs): # for all epochs
              ...
              for j in range (0,data.shape[0], batch_size):            
                ...
                #Display logs per epoch step
                  ...
                  # for validation:
                  result = sess.run([merge, loss], feed_dict={X:validation_set})
                  summary_str = result[0]
                  err = result[1]
                  writer_val.add_summary(summary_str, k)
                  print("Epoch:", '%04d' % (k+1), "cost=", \
                      "{:.9f}".format(avg_cost)," --- Validation cost at step %s: %s" % (k, err))
                '''
